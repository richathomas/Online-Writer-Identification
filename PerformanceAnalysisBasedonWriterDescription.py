...
#-----------------------------------------------------------------
    ...
    Accuraccy = []
    feature1 = np.array([F[:, 0]]).transpose()
    Sp = np.zeros_like(feature1, dtype='float')
    Sn = np.zeros_like(feature1, dtype='float')
    n, m = feature1.shape
    for i in range(n):
        for j in range(m):
            if feature1[i, j] >= (alpha[i,j]*fi[i,j]):
                Sp[i, j] = 1 / (1 + abs(feature1[i, j] - (alpha[i,j]*fi[i,j])))
            else:
                Sp[i, j] = 0

            if feature1[i, j] < (alpha[i,j]*fi[i,j]):
                Sn[i, j] = 1 / (1 + abs(feature1[i, j] - (alpha[i,j]*fi[i,j])))
            else:
                Sn[i, j] = 0
    ...
    Accuraccy.append(score)

    # -----------------------------------------------------------------
    feature1 = np.array([F[:, 1]]).transpose()
    Sp = np.zeros_like(feature1, dtype='float')
    Sn = np.zeros_like(feature1, dtype='float')
    n, m = feature1.shape
    for i in range(n):
...
# -----------------------------------------------------------------
    feature1 = np.array([F[:, 2]]).transpose()
    Sp = np.zeros_like(feature1, dtype='float')
    Sn = np.zeros_like(feature1, dtype='float')
    n, m = feature1.shape
    for i in range(n):
...
# -----------------------------------------------------------------
    feature1 = np.concatenate((np.array([F[:, 0]]).transpose(),np.array([F[:, 1]]).transpose()), axis=1)
    alpha1 =  np.concatenate((np.array([alpha[:, 0]]).transpose(),np.array([alpha[:, 1]]).transpose()), axis=1)
    Sp = np.zeros_like(feature1, dtype='float')
    Sn = np.zeros_like(feature1, dtype='float')
    n, m = feature1.shape
    for i in range(n):
        for j in range(m):
            if feature1[i, j] >= (alpha1[i, j] * fi[i, j]):
                Sp[i, j] = 1 / (1 + abs(feature1[i, j] - (alpha1[i, j] * fi[i, j])))
            else:
                Sp[i, j] = 0

            if feature1[i, j] < (alpha1[i, j] * fi[i, j]):
                Sn[i, j] = 1 / (1 + abs(feature1[i, j] - (alpha1[i, j] * fi[i, j])))
            else:
                Sn[i, j] = 0
...
# -----------------------------------------------------------------
    feature1 = np.concatenate(
        (np.array([F[:, 1]]).transpose(), np.array([F[:, 2]]).transpose()), axis=1)
    alpha1 = np.concatenate((np.array([alpha[:, 1]]).transpose(), np.array([alpha[:, 2]]).transpose()), axis=1)
    Sp = np.zeros_like(feature1, dtype='float')
    Sn = np.zeros_like(feature1, dtype='float')
    n, m = feature1.shape
    for i in range(n):
...
# -----------------------------------------------------------------
    feature1 = np.concatenate(
        (np.array([F[:, 0]]).transpose(), np.array([F[:, 2]]).transpose()), axis=1)
    alpha1 = np.concatenate((np.array([alpha[:, 0]]).transpose(), np.array([alpha[:, 2]]).transpose()), axis=1)
    Sp = np.zeros_like(feature1, dtype='float')
    Sn = np.zeros_like(feature1, dtype='float')
    n, m = feature1.shape
    for i in range(n):
...
# -----------------------------------------------------------------
    ...
    score = metrics.accuracy_score(Y_test, y_pred)
    Accuraccy.append(score)
    Accuraccy = [round(i * 100, 2) for i in Accuraccy]

# -----------------------------------------------------------------------------

    strlbl = ['P', 'D', 'A', 'P, D','D, A','P, A','P, D, A']
    plt.figure(1);
    plt.plot(strlbl, Accuraccy, color='orange', marker='o', markerfacecolor='black')
    plt.xlabel("Features")
    plt.ylabel('Performance')
    plt.title('Performance Analysis Based on Features')
    plt.show()
