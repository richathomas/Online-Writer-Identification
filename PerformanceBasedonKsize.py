...
# GLobal Features
    ...
    for k in range(50,501,50):
        alpha = generateorthogonalmatrix(F.shape[0],F.shape[1],k)
        ...
        accuraccy.append(round(score * 100, 2))
        print('For K Size: ', k, ' Accuraccy: ', round(score * 100, 2))

    labels = list(range(50,501,50))
    strlbl = [str(i) for i in labels]
    plt.figure(1);
    plt.plot(strlbl, accuraccy, color='orange', marker='o', markerfacecolor='black')
    plt.xlabel("K Size")
    plt.ylabel('Performance')
    plt.title('Performance Analysis Based on K Size')
    plt.show()
